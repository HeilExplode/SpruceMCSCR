...
public class ItemModelGenerators {
    ...
    private void generateArmorTrims(ArmorItem armorItem) {
        ...
        ResourceLocation resourceLocation3 = TextureMapping.getItemTexture(armorItem);
        ResourceLocation resourceLocation4 = TextureMapping.getItemTexture(armorItem, "_overlay");
        if (armorItem.getMaterial().is(ArmorMaterials.LEATHER)) {
            ModelTemplates.TWO_LAYERED_ITEM.create(resourceLocation2, TextureMapping.layered(resourceLocation3, resourceLocation4), this.output, (resourceLocation, map) -> this.generateBaseArmorTrimTemplate(resourceLocation, map, armorItem.getMaterial()));
        } else {
            ModelTemplates.FLAT_ITEM.create(resourceLocation2, TextureMapping.layer0(resourceLocation3), this.output, (resourceLocation, map) -> this.generateBaseArmorTrimTemplate(resourceLocation, map, armorItem.getMaterial()));
        }
        for (TrimModelData trimModelData : GENERATED_TRIM_MODELS) {
            ...
            ResourceLocation resourceLocation6 = ResourceLocation.withDefaultNamespace(string2).withPrefix("trims/items/");
            if (armorItem.getMaterial().is(ArmorMaterials.LEATHER)) {
    ...
    record TrimModelData(String name, float itemModelIndex, Map<Holder<ArmorMaterial>, String> overrideArmorMaterials) {
        public String name(Holder<ArmorMaterial> holder) {
            return this.overrideArmorMaterials.getOrDefault(holder, this.name);
        }
    }
}
