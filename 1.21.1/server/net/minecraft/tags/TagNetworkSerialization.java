...
public class TagNetworkSerialization {
    public static Map<ResourceKey<? extends Registry<?>>, NetworkPayload> serializeTagsToNetwork(LayeredRegistryAccess<RegistryLayer> layeredRegistryAccess) {
        return RegistrySynchronization.networkSafeRegistries(layeredRegistryAccess).map(registryEntry -> Pair.of(registryEntry.key(), (Object)TagNetworkSerialization.serializeToNetwork(registryEntry.value()))).filter(pair -> ((NetworkPayload)pair.getSecond()).size() > 0).collect(Collectors.toMap(Pair::getFirst, Pair::getSecond));
    }
    ...
    private static <T> NetworkPayload serializeToNetwork(Registry<T> registry) {
        ...
        registry.getTags().forEach(pair -> {
            ...
            hashMap.put(((TagKey)pair.getFirst()).location(), (IntList)intArrayList);
        });
        return new NetworkPayload(hashMap);
    }

    static <T> void deserializeTagsFromNetwork(ResourceKey<? extends Registry<T>> resourceKey, Registry<T> registry, NetworkPayload networkPayload, TagOutput<T> tagOutput) {
    ...
    public static final class NetworkPayload {
        ...
        public <T> void applyToRegistry(Registry<T> registry) {
            ...
            TagNetworkSerialization.deserializeTagsFromNetwork(registry.key(), registry, this, hashMap::put);
