    ...
    private <IS> TypeRewriteRule cap(Type<IS> type) {
        ...
        return this.fixTypeEverywhereTyped("EntityEquipmentToArmorAndHandFix", this.getInputSchema().getType(References.ENTITY), this.getOutputSchema().getType(References.ENTITY), typed -> {
            ...
            if (((Optional)object2).isPresent()) {
                Dynamic dynamic2;
                object = Stream.concat((Stream)((Optional)object2).get(), Stream.generate(() -> EntityEquipmentToArmorAndHandFix.lambda$cap$1((Dynamic)object3))).iterator();
                float f = ((Dynamic)object.next()).asFloat(0.0f);
                if (dynamic.get("HandDropChances").result().isEmpty()) {
                    dynamic2 = dynamic.createList(Stream.of(Float.valueOf(f), Float.valueOf(0.0f)).map(arg_0 -> ((Dynamic)dynamic).createFloat(arg_0)));
                    dynamic = dynamic.set("HandDropChances", dynamic2);
                }
                if (dynamic.get("ArmorDropChances").result().isEmpty()) {
                    dynamic2 = dynamic.createList(Stream.of(Float.valueOf(((Dynamic)object.next()).asFloat(0.0f)), Float.valueOf(((Dynamic)object.next()).asFloat(0.0f)), Float.valueOf(((Dynamic)object.next()).asFloat(0.0f)), Float.valueOf(((Dynamic)object.next()).asFloat(0.0f))).map(arg_0 -> ((Dynamic)dynamic).createFloat(arg_0)));
                    dynamic = dynamic.set("ArmorDropChances", dynamic2);
                }
                dynamic = dynamic.remove("DropChances");
            }
            return typed.set(opticFinder, type3, (Object)Pair.of((Object)either, (Object)Pair.of((Object)either2, (Object)Pair.of((Object)either3, (Object)dynamic))));
        });
    }

    private static /* synthetic */ Dynamic lambda$cap$1(Dynamic dynamic) {
