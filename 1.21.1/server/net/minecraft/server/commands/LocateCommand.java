...
public class LocateCommand {
    ...
    private static Optional<? extends HolderSet.ListBacked<Structure>> getHolders(ResourceOrTagKeyArgument.Result<Structure> result, Registry<Structure> registry) {
        return (Optional)result.unwrap().map(resourceKey -> registry.getHolder((ResourceKey<Structure>)resourceKey).map(holder -> HolderSet.direct(holder)), registry::getTag);
    }
    ...
    private static int locateStructure(CommandSourceStack commandSourceStack, ResourceOrTagKeyArgument.Result<Structure> result) throws CommandSyntaxException {
        ...
        HolderSet holderSet = LocateCommand.getHolders(result, registry).orElseThrow(() -> ERROR_STRUCTURE_INVALID.create((Object)result.asPrintable()));
    ...
    private static int showLocateResult(CommandSourceStack commandSourceStack, BlockPos blockPos, Pair<BlockPos, ? extends Holder<?>> pair, String string, boolean bl, String string2, Duration duration) {
        ...
        MutableComponent mutableComponent = ComponentUtils.wrapInSquareBrackets(Component.translatable("chat.coordinates", blockPos2.getX(), string3, blockPos2.getZ())).withStyle(style -> style.withColor(ChatFormatting.GREEN).withClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/tp @s " + blockPos2.getX() + " " + string3 + " " + blockPos2.getZ())).withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, Component.translatable("chat.coordinates.tooltip"))));
