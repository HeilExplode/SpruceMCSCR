...
implements ArgumentType<Holder<T>> {
    ...
    public Holder<T> parse(StringReader stringReader) throws CommandSyntaxException {
        Tag tag = ResourceOrIdArgument.parseInlineOrId(stringReader);
        if (!this.hasRegistry) {
            return null;
        }
        RegistryOps<Tag> registryOps = this.registryLookup.createSerializationContext(NbtOps.INSTANCE);
        return (Holder)this.codec.parse(registryOps, (Object)tag).getOrThrow(string -> ERROR_FAILED_TO_PARSE.createWithContext((ImmutableStringReader)stringReader, string));
    }
    ...
    static Tag parseInlineOrId(StringReader stringReader) throws CommandSyntaxException {
        ...
        if (ResourceOrIdArgument.hasConsumedWholeArg(stringReader)) {
            return tag;
        }
