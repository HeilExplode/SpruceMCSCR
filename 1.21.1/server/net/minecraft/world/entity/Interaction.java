...
Targeting {
    ...
    protected void readAdditionalSaveData(CompoundTag compoundTag) {
        ...
        if (compoundTag.contains(TAG_ATTACK)) {
            PlayerAction.CODEC.decode((DynamicOps)NbtOps.INSTANCE, (Object)compoundTag.get(TAG_ATTACK)).resultOrPartial(Util.prefix("Interaction entity", arg_0 -> ((Logger)LOGGER).error(arg_0))).ifPresent(pair -> {
        ...
        if (compoundTag.contains(TAG_INTERACTION)) {
            PlayerAction.CODEC.decode((DynamicOps)NbtOps.INSTANCE, (Object)compoundTag.get(TAG_INTERACTION)).resultOrPartial(Util.prefix("Interaction entity", arg_0 -> ((Logger)LOGGER).error(arg_0))).ifPresent(pair -> {
    ...
    protected void addAdditionalSaveData(CompoundTag compoundTag) {
        ...
        if (this.attack != null) {
            PlayerAction.CODEC.encodeStart((DynamicOps)NbtOps.INSTANCE, (Object)this.attack).ifSuccess(tag -> compoundTag.put(TAG_ATTACK, (Tag)tag));
        }
        if (this.interaction != null) {
            PlayerAction.CODEC.encodeStart((DynamicOps)NbtOps.INSTANCE, (Object)this.interaction).ifSuccess(tag -> compoundTag.put(TAG_INTERACTION, (Tag)tag));
        }
