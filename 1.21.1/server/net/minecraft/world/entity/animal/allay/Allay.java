...
VibrationSystem {
    ...
    public boolean hurt(DamageSource damageSource, float f) {
        Optional<UUID> optional = damageSource.getEntity();
        if (optional instanceof Player) {
            Player player = (Player)((Object)optional);
            optional = this.getBrain().getMemory(MemoryModuleType.LIKED_PLAYER);
    ...
    public void addAdditionalSaveData(CompoundTag compoundTag) {
        ...
        VibrationSystem.Data.CODEC.encodeStart(registryOps, (Object)this.vibrationData).resultOrPartial(string -> LOGGER.error("Failed to encode vibration listener for Allay: '{}'", string)).ifPresent(tag -> compoundTag.put("listener", (Tag)tag));
    ...
    public void readAdditionalSaveData(CompoundTag compoundTag) {
        ...
        if (compoundTag.contains("listener", 10)) {
            VibrationSystem.Data.CODEC.parse(registryOps, (Object)compoundTag.getCompound("listener")).resultOrPartial(string -> LOGGER.error("Failed to parse vibration listener for Allay: '{}'", string)).ifPresent(data -> {
