...
implements Shearable {
    ...
    private static final Map<DyeColor, Integer> COLOR_BY_DYE = Maps.newEnumMap(Arrays.stream(DyeColor.values()).collect(Collectors.toMap(dyeColor -> dyeColor, Sheep::createSheepColor)));
    ...
    private static int createSheepColor(DyeColor dyeColor) {
        ...
        float f = 0.75f;
        return FastColor.ARGB32.color(255, Mth.floor((float)FastColor.ARGB32.red(n) * 0.75f), Mth.floor((float)FastColor.ARGB32.green(n) * 0.75f), Mth.floor((float)FastColor.ARGB32.blue(n) * 0.75f));
    }
    ...
    public InteractionResult mobInteract(Player player, InteractionHand interactionHand) {
        ...
        if (itemStack.is(Items.SHEARS)) {
            if (!this.level().isClientSide && this.readyForShearing()) {
                ...
                itemStack.hurtAndBreak(1, player, Sheep.getSlotForHand(interactionHand));
    ...
    public static DyeColor getRandomSheepColor(RandomSource randomSource) {
        int n = randomSource.nextInt(100);
        if (n < 5) {
    ...
    private DyeColor getOffspringColor(Animal animal, Animal animal2) {
        ...
        CraftingInput craftingInput = Sheep.makeCraftInput(dyeColor, dyeColor2);
        return this.level().getRecipeManager().getRecipeFor(RecipeType.CRAFTING, craftingInput, this.level()).map(recipeHolder -> ((CraftingRecipe)recipeHolder.value()).assemble(craftingInput, this.level().registryAccess())).map(ItemStack::getItem).filter(DyeItem.class::isInstance).map(DyeItem.class::cast).map(DyeItem::getDyeColor).orElseGet(() -> this.level().random.nextBoolean() ? dyeColor : dyeColor2);
    }
