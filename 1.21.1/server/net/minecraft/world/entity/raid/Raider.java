...
extends PatrollingMonster {
    ...
    static final Predicate<ItemEntity> ALLOWED_ITEMS = itemEntity -> !itemEntity.hasPickUpDelay() && itemEntity.isAlive() && ItemStack.matches(itemEntity.getItem(), Raid.getLeaderBannerInstance(itemEntity.registryAccess().lookupOrThrow(Registries.BANNER_PATTERN)));
    @Nullable
    protected Raid raid;
    private int wave;
    ...
    public boolean isCaptain() {
        ...
        boolean bl = !itemStack.isEmpty() && ItemStack.matches(itemStack, Raid.getLeaderBannerInstance(this.registryAccess().lookupOrThrow(Registries.BANNER_PATTERN)));
    ...
    extends Goal {
        ...
        public boolean canUse() {
            ...
            if (!((Raider)this.mob).hasActiveRaid() || ((Raider)this.mob).getCurrentRaid().isOver() || !((PatrollingMonster)this.mob).canBeLeader() || ItemStack.matches(((Mob)this.mob).getItemBySlot(EquipmentSlot.HEAD), Raid.getLeaderBannerInstance(((Entity)this.mob).registryAccess().lookupOrThrow(Registries.BANNER_PATTERN)))) {
                return false;
            }
            Raider raider = raid.getLeader(((Raider)this.mob).getWave());
            if (!(raider != null && raider.isAlive() || (list = ((Entity)this.mob).level().getEntitiesOfClass(ItemEntity.class, ((Entity)this.mob).getBoundingBox().inflate(16.0, 8.0, 16.0), ALLOWED_ITEMS)).isEmpty())) {
        ...
        public void tick() {
            List<ItemEntity> list;
            if (((Mob)this.mob).getNavigation().getTargetPos().closerToCenterThan(((Entity)this.mob).position(), 1.414) && !(list = ((Entity)this.mob).level().getEntitiesOfClass(ItemEntity.class, ((Entity)this.mob).getBoundingBox().inflate(4.0, 4.0, 4.0), ALLOWED_ITEMS)).isEmpty()) {
    ...
    extends Goal {
        ...
        public HoldGroundAttackGoal(Raider raider, AbstractIllager abstractIllager, float f) {
