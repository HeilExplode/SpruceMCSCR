...
public class StackedContents {
    ...
    public int getBiggestCraftableStack(RecipeHolder<?> recipeHolder, int n, @Nullable IntList intList) {
        return new RecipePicker((Recipe<?>)recipeHolder.value()).tryPickAll(n, intList);
    }
    ...
    class RecipePicker {
        ...
        public boolean tryPick(int n, @Nullable IntList intList) {
            ...
            for (Ingredient ingredient : nonNullList) {
                if (n2 != 0 && ingredient.isEmpty()) {
                    intList.add(0);
                    continue;
                }
                for (int i = 0; i < this.itemCount; ++i) {
                    if (!this.hasResidual(false, n6, i)) continue;
                    this.toggleResidual(true, i, n6);
                    StackedContents.this.put(this.items[i], n);
                    if (n2 == 0) continue;
                    intList.add(this.items[i]);
                }
                ++n6;
            }
            return n3 != 0;
