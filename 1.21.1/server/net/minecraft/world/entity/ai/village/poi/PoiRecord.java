...
public class PoiRecord {
    ...
    public static Codec<PoiRecord> codec(Runnable runnable) {
        return RecordCodecBuilder.create(instance -> instance.group((App)BlockPos.CODEC.fieldOf("pos").forGetter(poiRecord -> poiRecord.pos), (App)RegistryFixedCodec.create(Registries.POINT_OF_INTEREST_TYPE).fieldOf("type").forGetter(poiRecord -> poiRecord.poiType), (App)Codec.INT.fieldOf("free_tickets").orElse((Object)0).forGetter(poiRecord -> poiRecord.freeTickets), (App)RecordCodecBuilder.point((Object)runnable)).apply((Applicative)instance, PoiRecord::new));
    }

    private PoiRecord(BlockPos blockPos, Holder<PoiType> holder, int n, Runnable runnable) {
