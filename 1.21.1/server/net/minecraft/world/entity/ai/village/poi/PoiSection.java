...
public class PoiSection {
    ...
    public static Codec<PoiSection> codec(Runnable runnable) {
        return RecordCodecBuilder.create(instance -> instance.group((App)RecordCodecBuilder.point((Object)runnable), (App)Codec.BOOL.lenientOptionalFieldOf("Valid", (Object)false).forGetter(poiSection -> poiSection.isValid), (App)PoiRecord.codec(runnable).listOf().fieldOf("Records").forGetter(poiSection -> ImmutableList.copyOf((Collection)poiSection.records.values()))).apply((Applicative)instance, PoiSection::new)).orElseGet(Util.prefix("Failed to read POI section: ", arg_0 -> ((Logger)LOGGER).error(arg_0)), () -> new PoiSection(runnable, false, (List<PoiRecord>)ImmutableList.of()));
    }
    ...
    public void refresh(Consumer<BiConsumer<BlockPos, Holder<PoiType>>> consumer) {
        if (!this.isValid) {
            ...
            this.clear();
            consumer.accept((arg_0, arg_1) -> this.lambda$refresh$8((Short2ObjectMap)short2ObjectOpenHashMap, arg_0, arg_1));
            this.isValid = true;
            this.setDirty.run();
        }
    }
    ...
    private /* synthetic */ void lambda$refresh$8(Short2ObjectMap short2ObjectMap, BlockPos blockPos, Holder holder) {
