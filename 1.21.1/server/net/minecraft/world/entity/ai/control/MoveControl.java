...
implements Control {
    ...
    public void tick() {
        if (this.operation == Operation.STRAFE) {
            ...
            f6 = f3 / f6;
            float f7 = Mth.sin(this.mob.getYRot() * ((float)Math.PI / 180));
            float f8 = Mth.cos(this.mob.getYRot() * ((float)Math.PI / 180));
            ...
            if (d3 > (double)this.mob.maxUpStep() && d * d + d2 * d2 < (double)Math.max(1.0f, this.mob.getBbWidth()) || !voxelShape.isEmpty() && this.mob.getY() < voxelShape.max(Direction.Axis.Y) + (double)blockPos.getY() && !blockState.is(BlockTags.DOORS) && !blockState.is(BlockTags.FENCES)) {
    ...
    private boolean isWalkable(float f, float f2) {
        ...
        return pathNavigation == null || (nodeEvaluator = pathNavigation.getNodeEvaluator()) == null || nodeEvaluator.getPathType(this.mob, BlockPos.containing(this.mob.getX() + (double)f, this.mob.getBlockY(), this.mob.getZ() + (double)f2)) == PathType.WALKABLE;
    }
