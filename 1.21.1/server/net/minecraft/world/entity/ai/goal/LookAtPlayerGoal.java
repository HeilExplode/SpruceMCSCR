...
extends Goal {
    ...
    public LookAtPlayerGoal(Mob mob, Class<? extends LivingEntity> clazz, float f, float f2, boolean bl) {
        ...
        this.lookAtContext = clazz == Player.class ? TargetingConditions.forNonCombat().range(f).selector(livingEntity -> EntitySelector.notRiding(mob).test((Entity)livingEntity)) : TargetingConditions.forNonCombat().range(f);
    }
