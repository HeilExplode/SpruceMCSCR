...
extends Behavior<Goat> {
    ...
    protected void tick(ServerLevel serverLevel, Goat goat, long l) {
        ...
        if (!list.isEmpty()) {
            ...
            if (livingEntity.hurt(damageSource = serverLevel.damageSources().noAggroMobAttack(goat), (float)goat.getAttributeValue(Attributes.ATTACK_DAMAGE))) {
                EnchantmentHelper.doPostAttackEffects(serverLevel, livingEntity, damageSource);
            }
            int n = goat.hasEffect(MobEffects.MOVEMENT_SPEED) ? goat.getEffect(MobEffects.MOVEMENT_SPEED).getAmplifier() + 1 : 0;
            int n2 = goat.hasEffect(MobEffects.MOVEMENT_SLOWDOWN) ? goat.getEffect(MobEffects.MOVEMENT_SLOWDOWN).getAmplifier() + 1 : 0;
            float f = 0.25f * (float)(n - n2);
            float f2 = Mth.clamp(goat.getSpeed() * 1.65f, 0.2f, 3.0f) + f;
            float f3 = livingEntity.isDamageSourceBlocked(serverLevel.damageSources().mobAttack(goat)) ? 0.5f : 1.0f;
            livingEntity.knockback((double)(f3 * f2) * this.getKnockbackForce.applyAsDouble(goat), this.ramDirection.x(), this.ramDirection.z());
