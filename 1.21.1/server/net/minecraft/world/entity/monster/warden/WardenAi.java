...
public class WardenAi {
    ...
    private static void initCoreActivity(Brain<Warden> brain) {
        brain.addActivity(Activity.CORE, 0, (ImmutableList<BehaviorControl<Warden>>)ImmutableList.of((Object)new Swim(0.8f), SetWardenLookTarget.create(), (Object)new LookAtTargetSink(45, 90), (Object)new MoveToTargetSink()));
    }
    ...
    private static void initFightActivity(Warden warden, Brain<Warden> brain) {
        brain.addActivityAndRemoveMemoryWhenStopped(Activity.FIGHT, 10, (ImmutableList<BehaviorControl<Warden>>)ImmutableList.of(DIG_COOLDOWN_SETTER, StopAttackingIfTargetInvalid.create(livingEntity -> !warden.getAngerLevel().isAngry() || !warden.canTargetEntity((Entity)livingEntity), WardenAi::onTargetInvalid, false), SetEntityLookTarget.create(livingEntity -> WardenAi.isTarget(warden, livingEntity), (float)warden.getAttributeValue(Attributes.FOLLOW_RANGE)), SetWalkTargetFromAttackTargetIfTargetOutOfReach.create(1.2f), (Object)new SonicBoom(), MeleeAttack.create(18)), MemoryModuleType.ATTACK_TARGET);
    }
