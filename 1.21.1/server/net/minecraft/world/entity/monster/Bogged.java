...
implements Shearable {
    ...
    public void addAdditionalSaveData(CompoundTag compoundTag) {
        ...
        compoundTag.putBoolean(SHEARED_TAG_NAME, this.isSheared());
    }

    @Override
    public void readAdditionalSaveData(CompoundTag compoundTag) {
        super.readAdditionalSaveData(compoundTag);
        this.setSheared(compoundTag.getBoolean(SHEARED_TAG_NAME));
    }
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand interactionHand) {
        ...
        if (itemStack.is(Items.SHEARS) && this.readyForShearing()) {
            ...
            if (!this.level().isClientSide) {
                itemStack.hurtAndBreak(1, player, Bogged.getSlotForHand(interactionHand));
            }
    ...
    private void spawnShearedMushrooms() {
        ...
        if (object instanceof ServerLevel) {
            ...
            object = serverLevel.getServer().reloadableRegistries().getLootTable(BuiltInLootTables.BOGGED_SHEAR);
            LootParams lootParams = new LootParams.Builder(serverLevel).withParameter(LootContextParams.ORIGIN, this.position()).withParameter(LootContextParams.THIS_ENTITY, this).create(LootContextParamSets.SHEARING);
            for (ItemStack itemStack : ((LootTable)object).getRandomItems(lootParams)) {
