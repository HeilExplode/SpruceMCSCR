...
public class PiglinAi {
    ...
    private static void initFightActivity(Piglin piglin, Brain<Piglin> brain) {
        brain.addActivityAndRemoveMemoryWhenStopped(Activity.FIGHT, 10, (ImmutableList<BehaviorControl<Piglin>>)ImmutableList.of(StopAttackingIfTargetInvalid.create(livingEntity -> !PiglinAi.isNearestValidAttackTarget(piglin, livingEntity)), BehaviorBuilder.triggerIf(PiglinAi::hasCrossbow, BackUpIfTooClose.create(5, 0.75f)), SetWalkTargetFromAttackTargetIfTargetOutOfReach.create(1.0f), MeleeAttack.create(20), new CrossbowAttack(), RememberIfHoglinWasKilled.create(), EraseMemoryIf.create(PiglinAi::isNearZombified, MemoryModuleType.ATTACK_TARGET)), MemoryModuleType.ATTACK_TARGET);
    }
    ...
    protected static void stopHoldingOffHandItem(Piglin piglin, boolean bl) {
        ...
        if (piglin.isAdult()) {
            ...
            boolean bl5;
            boolean bl6 = bl5 = !piglin.equipItemIfPossible(itemStack).isEmpty();
            if (!bl5) {
    ...
    private static Optional<? extends LivingEntity> findNearestValidAttackTarget(Piglin piglin) {
        ...
        if (optional2.isPresent() && Sensor.isEntityAttackableIgnoringLineOfSight(piglin, optional2.get())) {
            return optional2;
        }
    ...
    public static boolean isWearingGold(LivingEntity livingEntity) {
        Iterable<ItemStack> iterable = livingEntity.getArmorAndBodyArmorSlots();
        for (ItemStack itemStack : iterable) {
            Item item = itemStack.getItem();
            if (!(item instanceof ArmorItem) || !((ArmorItem)item).getMaterial().is(ArmorMaterials.GOLD)) continue;
            return true;
        }
        return false;
    }
