...
public final class TreeGrower {
    ...
    public boolean growTree(ServerLevel serverLevel, ChunkGenerator chunkGenerator, BlockPos blockPos, BlockState blockState, RandomSource randomSource) {
        ...
        if (resourceKey != null && (object = (Holder)serverLevel.registryAccess().registryOrThrow(Registries.CONFIGURED_FEATURE).getHolder(resourceKey).orElse(null)) != null) {
            for (int i = 0; i >= -1; --i) {
                for (int j = 0; j >= -1; --j) {
        ...
        Holder holder = serverLevel.registryAccess().registryOrThrow(Registries.CONFIGURED_FEATURE).getHolder((ResourceKey<ConfiguredFeature<?, ?>>)object).orElse(null);
        if (holder == null) {
            return false;
        }
