    ...
    extends TemplateStructurePiece {
        ...
        protected void handleDataMarker(String string, BlockPos blockPos, ServerLevelAccessor serverLevelAccessor, RandomSource randomSource, BoundingBox boundingBox) {
            if (string.startsWith("Chest")) {
                ...
                switch (string) {
                    case "Mage": {
                        arrayList.add(EntityType.EVOKER.create(serverLevelAccessor.getLevel()));
                        break;
                    }
                    case "Warrior": {
                        arrayList.add(EntityType.VINDICATOR.create(serverLevelAccessor.getLevel()));
                        break;
                    }
                    case "Group of Allays": {
                        int n = serverLevelAccessor.getRandom().nextInt(3) + 1;
                        for (int i = 0; i < n; ++i) {
                            arrayList.add(EntityType.ALLAY.create(serverLevelAccessor.getLevel()));
                        }
                        break;
                    }
                    default: {
