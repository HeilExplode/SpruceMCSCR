...
public interface Spawner {
    ...
    public static void appendHoverText(ItemStack itemStack, List<Component> list, String string) {
        Component component = Spawner.getSpawnEntityDisplayName(itemStack, string);
        if (component != null) {
            list.add(component);
        } else {
    ...
    @Nullable
    public static Component getSpawnEntityDisplayName(ItemStack itemStack, String string) {
        CompoundTag compoundTag = itemStack.getOrDefault(DataComponents.BLOCK_ENTITY_DATA, CustomData.EMPTY).getUnsafe();
        ResourceLocation resourceLocation = Spawner.getEntityKey(compoundTag, string);
        if (resourceLocation != null) {
            return BuiltInRegistries.ENTITY_TYPE.getOptional(resourceLocation).map(entityType -> Component.translatable(entityType.getDescriptionId()).withStyle(ChatFormatting.GRAY)).orElse(null);
        }
        return null;
    }
