...
public record ConditionalEffect<T>(T effect, Optional<LootItemCondition> requirements) {
    public static Codec<LootItemCondition> conditionCodec(LootContextParamSet lootContextParamSet) {
        return LootItemCondition.DIRECT_CODEC.validate(lootItemCondition -> {
            ...
            return collector.getReport().map(string -> DataResult.error(() -> "Validation error in enchantment effect condition: " + string)).orElseGet(() -> DataResult.success((Object)lootItemCondition));
        });
    }

    public static <T> Codec<ConditionalEffect<T>> codec(Codec<T> codec, LootContextParamSet lootContextParamSet) {
        return RecordCodecBuilder.create(instance -> instance.group((App)codec.fieldOf("effect").forGetter(ConditionalEffect::effect), (App)ConditionalEffect.conditionCodec(lootContextParamSet).optionalFieldOf("requirements").forGetter(ConditionalEffect::requirements)).apply((Applicative)instance, ConditionalEffect::new));
    }
