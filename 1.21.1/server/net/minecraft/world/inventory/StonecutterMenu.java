...
extends AbstractContainerMenu {
    ...
    private void setupRecipeList(Container container, ItemStack itemStack) {
        ...
        if (!itemStack.isEmpty()) {
            this.recipes = this.level.getRecipeManager().getRecipesFor(RecipeType.STONECUTTING, StonecutterMenu.createRecipeInput(container), this.level);
        }
    }
    ...
    void setupResultSlot() {
        if (!this.recipes.isEmpty() && this.isValidRecipeIndex(this.selectedRecipeIndex.get())) {
            ...
            ItemStack itemStack = recipeHolder.value().assemble(StonecutterMenu.createRecipeInput(this.container), (HolderLookup.Provider)this.level.registryAccess());
    ...
    public ItemStack quickMoveStack(Player player, int n) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            } else if (n == 0 ? !this.moveItemStackTo(itemStack2, 2, 38, false) : (this.level.getRecipeManager().getRecipeFor(RecipeType.STONECUTTING, new SingleRecipeInput(itemStack2), this.level).isPresent() ? !this.moveItemStackTo(itemStack2, 0, 1, false) : (n >= 2 && n < 29 ? !this.moveItemStackTo(itemStack2, 29, 38, false) : n >= 29 && n < 38 && !this.moveItemStackTo(itemStack2, 2, 29, false)))) {
                return ItemStack.EMPTY;
            }
            if (itemStack2.isEmpty()) {
