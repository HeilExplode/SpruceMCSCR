...
public enum FrontAndTop implements StringRepresentable
{
    ...
    public static FrontAndTop fromFrontAndTop(Direction direction, Direction direction2) {
        int n = FrontAndTop.lookupKey(direction, direction2);
    ...
    static {
        LOOKUP_TOP_FRONT = (Int2ObjectMap)Util.make(new Int2ObjectOpenHashMap(FrontAndTop.values().length), int2ObjectOpenHashMap -> {
            for (FrontAndTop frontAndTop : FrontAndTop.values()) {
                int2ObjectOpenHashMap.put(FrontAndTop.lookupKey(frontAndTop.front, frontAndTop.top), (Object)frontAndTop);
            }
        });
    }
}
