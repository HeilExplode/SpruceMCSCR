...
public final class DataComponentPatch {
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, DataComponentPatch> STREAM_CODEC = new StreamCodec<RegistryFriendlyByteBuf, DataComponentPatch>(){
        ...
        public DataComponentPatch decode(RegistryFriendlyByteBuf registryFriendlyByteBuf) {
            ...
            for (n = 0; n < n2; ++n) {
                ...
                Object t = dataComponentType.streamCodec().decode(registryFriendlyByteBuf);
        ...
        public void encode(RegistryFriendlyByteBuf registryFriendlyByteBuf, DataComponentPatch dataComponentPatch) {
            ...
            for (Reference2ObjectMap.Entry entry : Reference2ObjectMaps.fastIterable(dataComponentPatch.map)) {
                ...
                1.encodeComponent(registryFriendlyByteBuf, dataComponentType, ((Optional)object).get());
            }
            for (Reference2ObjectMap.Entry entry : Reference2ObjectMaps.fastIterable(dataComponentPatch.map)) {
    ...
    record PatchKey(DataComponentType<?> type, boolean removed) {
        public static final Codec<PatchKey> CODEC = Codec.STRING.flatXmap(string -> {
            ...
            if ((dataComponentType = BuiltInRegistries.DATA_COMPONENT_TYPE.get(resourceLocation = ResourceLocation.tryParse(string))) == null) {
